// proxy.ts — Next.js 16's renamed middleware file.
// Gates /admin/agents/* and /api/agents/* behind the admin password.
//
// Auth model: matches the existing /admin/page.tsx pattern. The /admin login
// drops a `kw_admin` cookie equal to ADMIN_PASSWORD on successful login. This
// proxy checks that cookie value matches the env var. If it doesn't, browser
// navigations get redirected to /admin?next=<original>, and API calls get a
// JSON 401.
//
// What this gates:
